...
t_arbol pilaExp[MAX_PILA],
		pilaComp[MAX_PILA],
		pilaCond[MAX_PILA],
		pilaListaSent[MAX_PILA],
		pilaProg[MAX_PILA],
		pilaCompAss[MAX_PILA];	
...
output : OUT p_abre identificador p_cierra 
{
	...
	if(fueDeclarado(auxNodo.tipo))
	{
		if (auxNodo.tipo == TIPO_STRING)
		{
			p_output=CrearNodo(datosNodo,CrearHoja(auxNodo.nombre,auxNodo.tipo),CrearHoja(HOJA_VACIA,TIPO_NINGUNO));
			printf("output\n");
		}
		else
			TerminarAplicacion(20, "No se pueden imprimir por pantalla variables reales o enteras");
	}
	else
...
{ 
	...
	tipo=tipoDelResultado(OP_SUMA,p_expresion_aux->info.tipo,p_termino->info.tipo);
	strcpy(datosNodo.contenido,"+");
	datosNodo.tipo=tipo;
	p_expresion = apilarExpresion(CrearNodo(datosNodo,p_expresion_aux,p_termino)) ;
};
...
{	
	...
	tipo=tipoDelResultado(OP_RESTA,p_expresion_aux->info.tipo,p_termino->info.tipo);
	strcpy(datosNodo.contenido,"-");
	datosNodo.tipo=tipo;
	p_expresion = apilarExpresion(CrearNodo(datosNodo,p_expresion_aux,p_termino));
};
expresion : termino 
{ 
	p_expresion=apilarExpresion(p_termino);
};
...
{	
	...
	strcpy(datosNodo.contenido,"*");
	datosNodo.tipo=tipo;
	p_termino=CrearNodo(datosNodo,p_termino,p_factor);
};
...
termino : factor {p_termino=p_factor;};
...
factor : p_abre expresion p_cierra {p_factor=p_expresion;};
...
{
	printf("ERROR %d: %s. Line %d", exitCode, description, cantLineas+1);
	LimpiarHeap();
	fclose(pfIn);
	fclose(pfArbol);
	fclose(pfCASM);
...
void Fin_Cte_String(void)
{
	...
	*(auxString + (strlen(auxString))) = '\0';

	info.nombre[0] = '\0';
	strcat(info.nombre, "_");
	strcat(info.nombre, token);
	info.nombre[1]='_';

	info.tipo=TIPO_STRING;						
	strcpy(info.valor.valCteStr, auxString);
...
void GenerarAssembler(t_arbol pArbol)
{
    ...
    int cantAux=0, cantIf=0, cantWhile=0, cantElse=0,cantCuerpo=0;
    char contFinal[20];
	t_info auxInfo;
	
    pfCASM=fopen("final.asm","wt");
   ...
   /* puts("Sub-arbol mas a la izquierda");
    ...
    while(pSubArbolIzq)
    {
        ...
        if(!strcmp(pSubArbolIzq->info.contenido,"WHILE"))
        {
			puts("WHILE");
...
void declararVariablesASM()
{
    ...
    fprintf(pfCASM,"\n;Variables del sistema:\n");
    fprintf(pfCASM,"\t MAX_TEXT equ 30\n"); /*Limite string*/
    fprintf(pfCASM,"\t ENDL db 0Dh, 0Ah, '$'\n"); /*Fin de linea*/
    fprintf(pfCASM,"\t _zero dd 0.0 \n"); /*Constante cero*/
    fprintf(pfCASM,"\t msgError1 db \"Error: division por cero\",'$' \n"); /*Constante cero*/
    fprintf(pfCASM,"\t msgError2 db \"Error: desbordamiento\",'$' \n"); /*Desbordamiento*/
    fprintf(pfCASM,"\t msgError3 db \"Error: indice fuera de rango\",'$' \n"); /*Limite de vector*/
    fprintf(pfCASM,"\t _cotaEnteros dd %d\n",MAX_INT); /*Desbordamiento*/
    ...
    for(i = 0; i < cantEnTabla; i++)
    {
        ...
        if(item.tipo==TIPO_INT||item.tipo==TIPO_REAL)
        {
			...
			if(item.valor.valCte > 0)
				if(item.tipo==TIPO_INT)
					fprintf(pfCASM,"%.0f",item.valor.valCte);
				else
					fprintf(pfCASM,"%.2f",item.valor.valCte);
			else
				fprintf(pfCASM,"0");
        ...
        else if(item.tipo==TIPO_STRING)
		{
			...
			if(item.longitud > 0)
			{
				...
				fprintf(pfCASM,"db MAX_TEXT dup (?),'$'\n",item.nombre);
		}
    }

    /*Variables auxiliares*/
    fprintf(pfCASM,"\n;Variables auxiliares:\n");
    fprintf(pfCASM,"AUXSTR db MAX_TEXT dup (?), '$'\n"); /*Auxiliar para strings*/
}
...
void comparacionAss(int *cantWhile,t_arbol pSubArbolIzq,const char *comparacionCad,const char *instruccion,int *cantIf,const char *comparacionSimb)
{
    ...
    if(!strcmp(nodoPadre->info.contenido,"&&")==0 && !strcmp(nodoPadre->info.contenido,"||")==0 )
    {
//        if(!strcmp(nodoPadre->info.contenido,"while"))
//        {
//            fprintf(pfCASM,"_while%d:\n",*cantWhile);
//            apilarInt(&pilaIntWhile,*cantWhile);
//            (*cantWhile)++;
//        }
        /*Salto en Assembler*/
        ...
        if(!strcmp(nodoPadre->info.contenido,"IF"))
        {
            if(!strcmp(nodoPadre->der->info.contenido,CUERPO_PROGRAMA))
                fprintf(pfCASM,"%s _else%d\n",instruccion,*cantIf);
            else
                fprintf(pfCASM,"%s _fin_if%d\n",instruccion,*cantIf);
            apilarIntIf(*cantIf);
...
void comprobarFinControl(t_arbol pSubArbolIzq)
{
    ...
    if(p_prg!=pSubArbolIzq)
    {
        ...
        /*Es el fin de un if sin else o fin de while*/
        else if(!strcmp(nodoPadre->info.contenido,CUERPO_PROGRAMA))
        {
		
			 /*Si es el fin de un if sin else*/
			if(!strcmp(nodoPadre->padre->info.contenido,"IF"))
			{
				ret=desapilarIntIf();
				fprintf(pfCASM,"_fin_if%d:\n",ret);
			}
			 /*Fin del while*/
			else if(!strcmp(nodoPadre->padre->info.contenido,"WHILE"))
			{
...
int devolverTipo(char *id)
{
    int i;
    char idBuscado[MAX_TOKEN];
	ts_t auxTabla; 
    strcpy(idBuscado,id);
    //puts(idBuscado);
    for(i=0;i<30;i++)
    {
        if(*(idBuscado+i)=='['||*(idBuscado+i)=='\0')
        {
            *(idBuscado+i)='\0';
            i=30;
        }
    ...
    for(i=0;i<cantEnTabla;i++)
    {
		ObtenerItemTS(&tabla, i, &auxTabla);
        if(!strcmp(auxTabla.nombre,idBuscado))
            return auxTabla.tipo;
    }
    return -1;
}
