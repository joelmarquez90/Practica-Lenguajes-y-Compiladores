...
void comparacionAssIEE(int *,t_arbol,const char *,const char *,int *);
   ...
   /* puts("Sub-arbol mas a la izquierda");
    ...
    while(pSubArbolIzq)
    {
		...
		else if(!strcmp(pSubArbolIzq->info.contenido,"INLIST"))
		{
			...
			escribInstrucComparar();				
			fprintf(pfCASM,"MOV RESUL_%d, eax\n", idVector); 
		...
		else if(!strcmp(pSubArbolIzq->info.contenido,"AND"))
        {
			...
			if(esIEE(pSubArbolIzq->izq->info.contenido) == OK)
			{
				...
				strcpy(salto,"JNE");
				fprintf(pfCASM, "mov eax, RESUL_%d\n",pSubArbolIzq->izq->info.tipo);
			}
			else
			{				
...
void comparacionAssIEE(int *cantWhile, t_arbol pSubArbolIzq, const char *comparacionCad, const char *instruccion, int *cantIf)
{
