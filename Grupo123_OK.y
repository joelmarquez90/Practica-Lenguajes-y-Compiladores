...

{
	...
	if(p_expresion_aux->info.tipo!=TIPO_STRING) 
	{
		...
		TerminarAplicacion(40, "Error De Sintaxis. Constante string no es un elemento valido de la lista de variables");
};
...

{
	...
	printf("min_lista\n");
	p_expresion_aux=desapilarExpresion();                     
	if(p_expresion_aux->info.tipo!=TIPO_STRING && p_expresion_aux->info.tipo== p_lista_expresiones->info.tipo ) // SE AGREGA VALIDACION DE TIPOS
	{
...
void Ini_Cte_Real(void)
{
    ...
    strcat(token,&c);
    DEBUG ? printf("este es el primer digito de la cte real , %s",token) : printf("");
    DEBUG ? printf("este es el valor de constante al empezar real %f",cte) : printf("");
    cantDec=0;
}
...
void GenerarAssembler(t_arbol pArbol)
{
    ...
    if(!pfCASM || !pfDASM)
    {
        puts("Error: No se pudo crear los archivos necesarios para generar Assembler\n");
        getch();
        exit(1);
    }
