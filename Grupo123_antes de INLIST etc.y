...
ts_t info;
tabla_simb_t tabla;
int cantNodos=0, finArchivo=0, carRetornado=0, cantLineas=1, cantEnTabla=0, cantCteString=0;
   ...
   /* puts("Sub-arbol mas a la izquierda");
    ...
    while(pSubArbolIzq)
    {
        ...
        if(!strcmp(pSubArbolIzq->info.contenido,"WHILE"))
        {
            ...
            cantWhile++;
			/////////////AGREGADO
			if (pSubArbolIzq->padre == NULL)
			{
				puts("Soy un WHILE sin padre");
				strcpy(pSubArbolIzq->info.contenido,CUERPO_PROGRAMA);
			}
			////////////////////
        }
        ...
        {
			puts("!strcmp(pSubArbolIzq->info.contenido,\"==\")");
            t_arbol arbolAux;
			auxInfo.tipo=pSubArbolIzq->info.tipo;
			auxInfo.id=pSubArbolIzq->info.id;
			strcpy(auxInfo.contenido,pSubArbolIzq->info.contenido);
		...
		{
			puts("comprobarFinControl(pSubArbolIzq); CUERPO_PROGRAMA");
			comprobarFinControl(pSubArbolIzq);
		}
		  /*Liberar hojas*/
...
void comprobarFinControl(t_arbol pSubArbolIzq)
{
    ...
    if(p_prg != pSubArbolIzq)
    {
        ...
        {		
			puts("!strcmp(nodoPadre->info.contenido,CUERPO_PROGRAMA)");
			 /*Si es el fin de un if sin else*/
			if(nodoPadre->padre != NULL && !strcmp(nodoPadre->padre->info.contenido,"IF"))
			{
				puts("nodoPadre->padre != NULL && !strcmp(nodoPadre->padre->info.contenido,\"IF\")");
				ret=desapilarIntIf();
				fprintf(pfCASM,"_fin_if%d:\n",ret);
			}
			 /*Fin del while*/
			else if(nodoPadre->padre != NULL && !strcmp(nodoPadre->padre->info.contenido,"WHILE"))
			{
				puts("nodoPadre->padre != NULL !strcmp(nodoPadre->padre->info.contenido,\"WHILE\")");
